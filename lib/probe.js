 ...
 * - v2.4 修复：直接 spawn powershell.exe（不用 shell:true）+ taskkill /F /T 杀进程树
 * - v2.4 优化：feishu-ws 30s 内部缓存，避免每 5s spawn 进程
 */
...
/**
 * 杀整个进程树（包括孙子进程）
 * - 用 Windows 自带 taskkill /F /T，无需额外 npm 包
 */
...
/**
 * CLI 探活（v2.4 修复：直接 spawn，进程树清理）
 * - 当 cmd 是 PowerShell 时，不要用 shell:true，直接 spawn powershell.exe
 * - 超时时用 taskkill /F /T 杀进程树（防止孤儿 PowerShell 累积）
 */
function probeCli(cmd, timeoutMs = 5000) {
  return new Promise((resolve) => {
    ...
    // 判断是否是 PowerShell 命令
    const isPowerShell = /^\s*powershell(\.exe)?\s+/i.test(cmd);
    ...
    if (isPowerShell) {
      ...
      const exe = parts[0];
      // 剥掉每个参数外层的引号（spawn 不像 shell 那样自动处理引号）
      const args = parts.slice(1).map(a => a.replace(/^"|"$/g, ''));
      proc = spawn(exe, args, { windowsHide: true, stdio: ['ignore', 'pipe', 'pipe'] });
    } else {
      proc = spawn(cmd, { shell: true, windowsHide: true, stdio: ['ignore', 'pipe', 'pipe'] });
    }
...
/**
 * 飞书 WebSocket 探活（v2.4 优化：30s 内部缓存）
 * - WebSocket 连接稳定，不需要 5 秒探活
 * - 缓存结果，30 秒内复用
 */
let _feishuCache = { result: null, checkedAt: 0 };
const FEISHU_CACHE_MS = 30000; // 30 秒内部缓存

async function probeFeishuWs(timeoutMs = 8000) {
  const now = Date.now();

  // 命中缓存
  if (_feishuCache.result && (now - _feishuCache.checkedAt) < FEISHU_CACHE_MS) {
    // 返回副本，但标记为 cached
    return Object.assign({}, _feishuCache.result, {
      cached: true,
      latencyMs: 0, // 缓存无延迟
    });
  }
  ...
  // 写缓存
  _feishuCache = { result: fullResult, checkedAt: now };
...
module.exports = { probeService, probeHttp, probeCli, probeWslCli, probeFeishuWs, probeSubsystem, killProcessTree, explainStatus };